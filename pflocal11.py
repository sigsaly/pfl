...
class MapUI(QWidget):
    ...
    def moveButtonClicked(self):
        global pf, rs_pf, robot, move_done, weight, none_rs
        ...
        for i in range(num_pf):
            p.append( pf[i].move(move_distance, deg(move_theta)))
        ...
        if mw > rs_thld or none_rs == False:
            none_rs = False
            rs_pf = []
            for i in range(num_pf):
        ...
        #uncomment below to move pfs even when they're not resampled
        #else:
        #    pf = p
...
if __name__ == "__main__":
    ...
    def RSThresholdChanged():
        ...
        if 0 <= value and value < 10:
            ...
            rs_thld = 1.0E-4
        label6_1.setText('{:3.1E}'.format(rs_thld))        
    ...
    label6 = QLabel("Resample Threshold")
    label6_1 = QLabel(str(rs_thld))
    ...
    slider6 = QSlider(Qt.Horizontal) # resample threshold
    slider6.setValue(50)
    slider6.setSingleStep(10)
    slider6.valueChanged.connect(RSThresholdChanged)
    ...
    ll6 = QHBoxLayout()
    ll6.addWidget(label6)
    ll6.addWidget(label6_1)
    ...
    layout_s2.addLayout(ll6)
    layout_s2.addWidget(slider6)

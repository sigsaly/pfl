# pflocal7.py
# add bitmap data and test collision
...
class Robot:
    ...
    def sense(self):
        ...
        while 1:
            x = cCX(self.x + (cos(theta) * dist))
            y = cCY(self.y + (sin(theta) * dist))
        ...
        dist = dist + random.gauss(0.0, sensor_noise)
        return dist
    ...
    def getProbability(self, robot_dist):
        global sensor_noise        
        dist = self.sense()
        prob = self.calcGaussianProbability(robot_dist, sensor_noise, dist)
        return prob

...
class Robot:
    ...
    def sense(self):
        ...
        theta1 = self.theta
        theta2 = self.theta - sensor_angle + 2.0 * pi
        theta3 = self.theta + sensor_angle
        ...
        while True:
            x = cCX(self.x + (cos(theta1) * dist))
            y = cCY(self.y + (sin(theta1) * dist))
        ...
        dist = 10
        while True:
            x = cCX(self.x + (cos(theta2) * dist))
            y = cCY(self.y + (sin(theta2) * dist))
        ...
        dist = 10
        while True:
            x = cCX(self.x + (cos(theta3) * dist))
            y = cCY(self.y + (sin(theta3) * dist))
        ...
        dist1 = dist1 + random.gauss(0.0, sensor_noise)
        dist2 = dist2 + random.gauss(0.0, sensor_noise)
        dist3 = dist3 + random.gauss(0.0, sensor_noise)
    ...
    def getProbability(self, robot_dists):
        ...
        dists = self.sense() 
        prob1 = self.calcGaussianProbability(dists[0], sensor_noise, robot_dists[0])
        prob2 = self.calcGaussianProbability(dists[1], sensor_noise, robot_dists[1])
        prob3 = self.calcGaussianProbability(dists[2], sensor_noise, robot_dists[2])
...
class MapUI(QWidget):
    ...
    def paintEvent(self, event=None):
        ...
        if (robot.x1 != 0) and (robot.y1 != 0):
            painter.drawLine(pos_x, pos_y, robot.x1, robot.y1)        
        if (robot.x2 != 0) and (robot.y2 != 0):
            painter.drawLine(pos_x, pos_y, robot.x2, robot.y2)
        if (robot.x3 != 0) and (robot.y3 != 0):
            painter.drawLine(pos_x, pos_y, robot.x3, robot.y3)        
